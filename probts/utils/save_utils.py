...
def load_checkpoint(
    Model, checkpoint_path, scaler=None, learning_rate=None, no_training=False, **kwargs
):
    # Load the checkpoint
    checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
    # Extract the arguments for the forecaster
    forecaster_args = checkpoint["hyper_parameters"].get("forecaster", {})
    ...
    if isinstance(forecaster_args, Forecaster):
        ...
        if forecaster_args and "class_path" in forecaster_args:
            module_path, class_name = forecaster_args["class_path"].rsplit(".", 1)
        else:
        ...
        if forecaster_args and "init_args" in forecaster_args:
            forecaster_args = forecaster_args["init_args"]
        else:
            print(
                "Warning: No forecaster init_args found in checkpoint, using default PatchTST config"
            )
            forecaster_args = {
                "stride": 8,
                "patch_len": 16,
